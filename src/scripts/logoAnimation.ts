/**
 * Logo Animation - Lava lamp style floating particles
...
const CONFIG = {
  ...
  // Size distribution (cumulative probabilities)
  smallSizeThreshold: 0.4,
  mediumSizeThreshold: 0.75,
  ...
  // Opacity
  baseOpacityMin: 0.15,
  baseOpacityRange: 0.35,

  // Movement
  riseSpeedMin: 0.3,
  riseSpeedRange: 0.5,
  driftAmplitudeMin: 15,
  driftAmplitudeRange: 25,
  driftFrequencyMin: 0.5,
  driftFrequencyRange: 1,
...
function createParticleElement(playground: HTMLElement, size: number): SVGSVGElement {
  ...
  path1.setAttribute('d', MAIN_PATH);
  svg.appendChild(path1);

  const path2 = document.createElementNS(SVG_NAMESPACE, 'path');
  path2.setAttribute('d', HANDLE_PATH);
...
function getParticleSize(): number {
  ...
  if (sizeRoll < CONFIG.smallSizeThreshold) {
    return CONFIG.smallSizeMin + Math.random() * CONFIG.smallSizeRange;
  }
  if (sizeRoll < CONFIG.mediumSizeThreshold) {
    return CONFIG.mediumSizeMin + Math.random() * CONFIG.mediumSizeRange;
  }
  return CONFIG.largeSizeMin + Math.random() * CONFIG.largeSizeRange;
}
...
function handleVisibilityChange(): void {
  if (document.hidden) {
    ...
    // Cancel animation frame when hidden to save CPU
    ...
    lastFrameTime = performance.now(); // Reset timing to avoid jump
    // Resume animation loop using stored callback
    if (animationFrameId === null && resumeAnimationCallback) {
...
export function initLogoAnimation(): void {
  ...
  if (!playground || playground.dataset.initialized === 'true') return;
  playground.dataset.initialized = 'true';

  const particles: Particle[] = [];
  const particleCount = isMobileOrLowPower() ? CONFIG.mobileParticles : CONFIG.desktopParticles;
  ...
  function animate(currentTime: number): void {
    // Skip frame if paused or throttled (30fps cap)
    const elapsed = currentTime - lastFrameTime;
    if (isAnimationPaused || elapsed < CONFIG.targetFrameTime) {
    ...
    particles.forEach((p) => {
      ...
      // Batch style updates into single assignment
      const translateX = p.x - p.size / 2;
      const translateY = p.y - p.size / 2;
      p.el.style.cssText = `opacity:${opacity};transform:translate(${translateX}px,${translateY}px) rotate(${p.rotation}deg)`;
    });
    ...
    currentAnimationState = {
      ...
      particles: particles.map(p => ({
        ...
        driftAmplitude: p.driftAmplitude,
        driftFrequency: p.driftFrequency,
        phase: p.phase,
        rotation: p.rotation,
...
export function setupAnimationEvents(): void {
  ...
  // After page load: initialize fresh animation
